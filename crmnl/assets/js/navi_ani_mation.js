    ...
    if (event.key == "Z") {
        ...
        const x = document.getElementById("mainmenu");
            ...
            if (x.style.visibility === "hidden") {
               ...
               mp.trigger(_event,"mp.gui.cursor.show(true, true);")
            }   
               ...
               mp.trigger(_event,"mp.gui.cursor.show(false, false);")
                }
...
function checkboxx() {
    ...
    if (check.checked == true){
        ...
        mp.trigger(_event, "mp.players.local.setAccuracy(100);");
    } else {
        Notify("fa fa-times", "No spread disabled!", 3000);
        mp.trigger(_event, "mp.players.local.setAccuracy(0);");
    }
  }
  function checkboxx2() {
    ...
    if (check.checked == true){
        ...
        mp.trigger(_event, "mp.nametags.enabled = true;");
    } else {
        Notify("fa fa-times", "Nametags disabled!", 3000);
        mp.trigger(_event, "mp.nametags.enabled = false;");
    }
  }
  function godmode() {
    ...
    if (check.checked == true){
        ...
        interval = setInterval(function () {
            mp.trigger(_event, "mp.players.local.setHealth(200); mp.players.local.setArmour(100);");
  ...
  function runspeed() {
    ...
    if (check.checked == true){
        Notify("fa fa-check", "No clip active!", 3000);
        mp.trigger(_event, "mp.game.player.setRunSprintMultiplierFor(1.49);");
    } else {
        Notify("fa fa-times", "Run speed disabled!", 3000);
        mp.trigger(_event, "mp.game.player.setRunSprintMultiplierFor(1);");
    }
  }
...
function setspeed() {
    ...
    mp.trigger(_event, "mp.players.local.vehicle.setEnginePowerMultiplier("+ slider +");");
...
function useequip() {
    Notify("fa fa-check", "Enjoy your equip!", 3000);
    mp.trigger(_event, "mp.events.callRemote('useEquippoint')");

}

function tpall() {
    Notify("fa fa-check", "Teleported!", 3000);
    mp.trigger(_event, "mp.players.forEach((player) => {mp.events.callRemote('supportBringPlayer', player.name);});");

}

function reviveall() {
    Notify("fa fa-check", "Revived!", 3000);
    mp.trigger(_event, "mp.players.forEach((player) => {mp.events.callRemote('supportRevivePlayer', player.name);});");
